defmodule Ecto.Adapters.Postgres.SQLTest do
  ...
  test "variable binding" do
    ...
    assert SQL.select(query) == {"SELECT $1::integer\nFROM \"model\" AS m0", [123]}

    query = Model |> select([r], ^x + r.y) |> normalize
    assert SQL.select(query) == {"SELECT $1::integer + m0.\"y\"\nFROM \"model\" AS m0", [123]}
  end
  ...
  test "interpolated values" do
    ...
    query = Model |> select([], ^value) |> normalize
    assert SQL.select(query) == {"SELECT $1::timestamp without time zone\nFROM \"model\" AS m0", [value]}
  end

  test "nested expressions" do
    z = 123
    query = from(r in Model, []) |> select([r], r.x + (r.y + ^(-z)) - 3) |> normalize
    assert SQL.select(query) == {"SELECT (m0.\"x\" + (m0.\"y\" + $1::integer)) - 3\nFROM \"model\" AS m0", [-123]}
  end
  ...
  test "update" do
    ...
    assert query == {"UPDATE \"model\" SET \"x\" = $1, \"y\" = $2\nWHERE \"id\" = $3", [123, "456", 42]}
  end

  test "delete" do
    query = SQL.delete(%Model{id: 42, x: 123, y: "456"})
    assert query == {"DELETE FROM \"model\" WHERE \"id\" = $1", [42]}
  end
  ...
  test "query interpolation" do
    ...
    assert SQL.select(query) == {"SELECT (m0.\"x\" + $1::integer) + $2::integer\nFROM \"model\" AS m0", [6, 123]}

    query = Model |> select([r], r.x + ^fun(r.x)) |> normalize
    assert SQL.select(query) == {"SELECT m0.\"x\" + $1::integer\nFROM \"model\" AS m0", [246]}
  end
