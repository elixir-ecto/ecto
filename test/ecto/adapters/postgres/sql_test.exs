defmodule Ecto.Adapters.Postgres.SQLTest do
  ...
  test "binary ops" do
    ...
    query = Model |> select([r], r.x / 2) |> normalize
    assert SQL.select(query) == {~s{SELECT m0."x" / 2::numeric\nFROM "model" AS m0}, []}
  ...
  test "literals" do
    ...
    assert SQL.select(query) == {~s{SELECT 123.0::float\nFROM "model" AS m0}, []}
  end
  ...
  test "interpolated values" do
    ...
    assert SQL.select(query) == {~s{SELECT $1::decimal\nFROM "model" AS m0}, [value]}

    value = %Ecto.DateTime{year: 2014, month: 1, day: 16, hour: 20, min: 26, sec: 51}
    query = Model |> select([], ^value) |> normalize
    assert SQL.select(query) == {~s{SELECT $1::timestamp without time zone\nFROM "model" AS m0}, [value]}
  end
  ...
  test "update" do
    ...
    assert query == {~s{UPDATE "model" SET "x" = $1, "y" = $2\nWHERE "id" = $3}, [123, "456", 42]}
  end
  ...
  test "functions" do
    ...
    assert SQL.select(query) == {~s{SELECT round(12.34::float)\nFROM "model" AS m0}, []}

    query = Model |> select([], round(12.34, 1)) |> normalize
    assert SQL.select(query) == {~s{SELECT round(12.34::float, 1)\nFROM "model" AS m0}, []}
