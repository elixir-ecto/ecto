...
defmodule Ecto.Adapters.MySQLTest do
  ...
  defp normalize(query, operation \\ :all, counter \\ 0) do
    {query, _params, _key} =
      Ecto.Query.Planner.prepare(query, operation, Ecto.Adapters.MySQL, counter)
  ...
  test "distinct" do
    ...
    assert_raise Ecto.QueryError,
                 ...
                 fn ->
                   query =
                     Schema |> distinct([r], [r.x, r.y]) |> select([r], {r.x, r.y}) |> normalize
  ...
  test "interpolated values" do
    ...
    result =
      "SELECT s0.`id`, ? FROM `schema` AS s0 INNER JOIN `schema2` AS s1 ON ? " <>
        ...
        "GROUP BY ?, ? HAVING (?) AND (?) " <> "ORDER BY ?, s0.`x` LIMIT ? OFFSET ?"
  ...
  test "update all" do
    ...
    assert update_all(query) == ~s{UPDATE `schema` AS s0 SET s0.`x` = 0 WHERE (s0.`x` = 123)}
  ...
  test "cross join" do
    ...
    assert all(query) == "SELECT s0.`id`, s1.`id` FROM `schema` AS s0 CROSS JOIN `schema2` AS s1"
  end
  ...
  test "insert with on duplicate key" do
    ...
    assert query ==
             ~s{INSERT INTO `schema` (`x`,`y`) VALUES (?,?) ON DUPLICATE KEY UPDATE `z` = ?}
    ...
    assert_raise ArgumentError,
                 "The :conflict_target option is not supported in insert/insert_all by MySQL",
                 fn ->
  ...
  test "create table with a map column, and an empty map default" do
    ...
    assert execute_ddl(create) == [
             ~s|CREATE TABLE `posts` (`a` text DEFAULT '{}') ENGINE = INNODB|
           ]
  end
  ...
  test "create table with a map column, and a map default with values" do
    ...
    assert execute_ddl(create) == [
             ~s|CREATE TABLE `posts` (`a` text DEFAULT '{"baz":"boom","foo":"bar"}') ENGINE = INNODB|
           ]
  end
  ...
  test "create table with a map column, and a string default" do
    ...
    assert execute_ddl(create) == [
             ~s|CREATE TABLE `posts` (`a` text DEFAULT '{"foo":"bar","baz":"boom"}') ENGINE = INNODB|
           ]
  end
  ...
  test "create index" do
    ...
    assert execute_ddl(create) == [~s|CREATE INDEX `posts$main` ON `posts` (permalink(8))|]
  end
