...
defmodule Ecto.Query.BuilderTest do
  ...
  test "escape json_extract_path" do
    ...
    actual =
      escape(
        quote do
          json_extract_path(field(x, :y), ["a", "b"])
        end,
        [x: 0],
        __ENV__
      )
    ...
    actual =
      escape(
        quote do
          json_extract_path(parent_as(:x).y, ["a", "b"])
        end,
        [],
        __ENV__
      )
    ...
    assert_raise Ecto.Query.CompileError,
                 "expected JSON path to be a literal list or interpolated value, got: `bad`",
                 fn ->
                   escape(
                     quote do
  ...
  test "escape parameterized types" do
    parameterized_type =
      Macro.escape(Ecto.ParameterizedType.init(ParameterizedPrefixedString, prefix: "p"))
  ...
  test "escape raise" do
    ...
    assert_raise Ecto.Query.CompileError,
                 ~r"expected literal atom or string or interpolated value.*got: `var`",
                 fn ->
