...
defmodule Ecto.Query.SubqueryTest do
  ...
  test "prepare: subqueries" do
    ...
    assert params == []
    assert key == [:all, 0, [:all, 0, {"posts", Post, 127_044_068}]]
  ...
  test "prepare: subqueries with association joins" do
    ...
    assert_raise Ecto.QueryError, message, fn ->
      prepare(
        from(p in subquery(from p in Post, select: p.title), join: c in assoc(p, :comments))
      )
    end
  end
  ...
  describe "prepare: subqueries select" do
    ...
    test "raises on custom expressions" do
      ...
      assert_raise Ecto.SubQueryError,
                   ~r/subquery must select a source \(t\), a field \(t\.field\) or a map/,
                   fn ->
                     prepare(from(subquery(query), []))
                   end
    end
  ...
  test "prepare: wraps subquery errors" do
    ...
    assert Exception.message(exception) =~
             "from p in subquery(from p in Ecto.Query.SubqueryTest.Post"
  end
