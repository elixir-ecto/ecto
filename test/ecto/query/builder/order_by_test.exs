defmodule Ecto.Query.Builder.OrderByTest do
  ...
  describe "escape" do
    ...
    test "handles expressions and params" do
      ...
      assert {Macro.escape(quote(do: [asc: &0.x()])), {[], %{}}} ==
               escape(:order_by, quote(do: :x), {[], %{}}, [x: 0], __ENV__)

      assert {Macro.escape(quote(do: [asc: &0.x(), desc: &0.y()])), {[], %{}}} ==
               escape(:order_by, quote(do: [:x, desc: :y]), {[], %{}}, [x: 0], __ENV__)

      assert {Macro.escape(quote(do: [asc_nulls_first: &0.x(), desc_nulls_first: &0.y()])),
              {[], %{}}} ==
               escape(
                 :order_by,
                 quote(do: [asc_nulls_first: :x, desc_nulls_first: :y]),
                 {[], %{}},
                 [x: 0],
                 __ENV__
               )

      assert {Macro.escape(quote(do: [asc_nulls_last: &0.x(), desc_nulls_last: &0.y()])),
              {[], %{}}} ==
               escape(
                 :order_by,
                 quote(do: [asc_nulls_last: :x, desc_nulls_last: :y]),
                 {[], %{}},
                 [x: 0],
                 __ENV__
               )
      ...
      assert {Macro.escape(quote(do: [asc: 1 > 2])), {[], %{}}} ==
               escape(:order_by, quote(do: 1 > 2), {[], %{}}, [], __ENV__)

      assert {Macro.escape(
                quote(
                  do: [
                    desc: &0.id(),
                    asc: fragment({:raw, "lower("}, {:expr, &0.title()}, {:raw, ")"}),
                    asc: nth_value(&0.links(), 1)
                  ]
                )
              ), {[], %{}}} ==
               escape(:order_by, quote(do: my_complex_order(x)), {[], %{}}, [x: 0], __ENV__)
      ...
      assert {Macro.escape(
               ...
               escape(:order_by, quote(do: my_custom_order(x)), {[], %{}}, [x: 0], __ENV__)

      assert assert {[asc: {:{}, [], [:is_nil, [], [{:{}, [], [:&, [], [0]]}]]}], {[], %{}}} ==
                      escape(:order_by, quote(do: is_nil(x)), {[], %{}}, [x: 0], __ENV__)
    end

    test "raises on unbound variables" do
      assert_raise Ecto.Query.CompileError, ~r"unbound variable `x` in query", fn ->
        escape(:order_by, quote(do: x.y), {[], %{}}, [], __ENV__)
      end
    end
    ...
    test "raises on unknown expression" do
      ...
      assert_raise Ecto.Query.CompileError, message, fn ->
        escape(:order_by, quote(do: [test: x.y]), {[], %{}}, [x: 0], __ENV__)
      end
    end
    ...
    test "can reference the alias of a selected value with selected_as/1" do
      ...
      assert [asc: {:+, [], [{:selected_as, [], [:ident]}, {:selected_as, [], [:ident2]}]}] =
               hd(query.order_bys).expr
    end
    ...
    test "raises if name given to selected_as/1 is not an atom" do
      ...
      assert_raise Ecto.Query.CompileError, message, fn ->
        escape(
          :order_by,
          quote(do: [desc: selected_as("ident")]),
          {[], %{}},
          [],
          __ENV__
        )
  ...
  describe "at runtime" do
    test "accepts fields, lists or keyword lists" do
      ...
      assert [%{expr: [asc: {{:., _, [{:&, _, [0]}, :title]}, _, []}]}] =
               order_by("q", [q], asc: q.title).order_bys

      assert [%{expr: [asc: {{:., _, [{:&, _, [0]}, :title]}, _, []}]}] =
               order_by("q", [q], asc: q.title).order_bys

      assert [%{expr: [desc: {{:., _, [{:&, _, [0]}, :title]}, _, []}]}] =
               order_by("q", [q], desc: ^key).order_bys

      assert [%{expr: [desc: {{:., _, [{:&, _, [0]}, :title]}, _, []}]}] =
               order_by("q", [q], [{^dir, ^key}]).order_bys
    end
