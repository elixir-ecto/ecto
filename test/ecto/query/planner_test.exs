...
defmodule Ecto.Query.PlannerTest do
  ...
  test "plan: generates a cache key" do
    ...
    assert key == [:all, {"posts", Post, 69631610, "my_prefix"}]
  ...
  test "normalize: validate fields in embed_extract_path/2" do
    ...
    assert_raise RuntimeError, "cannot use `0` to refer to a field on an `embeds_one`", fn ->
      query = from(Post, []) |> select([p], p.meta[0])
      normalize(query)
    end

    query = from(Post, []) |> select([p], p.metas[0])
    normalize(query)

    query = from(Post, []) |> select([p], p.metas[0].author)
    normalize(query)

    assert_raise RuntimeError, "cannot use `:author` to refer to an item in `embeds_many`", fn ->
      query = from(Post, []) |> select([p], p.metas.author)
      normalize(query)
    end
  end
  ...
  test "normalize: select" do
    ...
    assert query.select.fields ==
           select_fields([:id, :post_title, :text, :code, :posted, :visits, :links, :meta, :metas], 0)
