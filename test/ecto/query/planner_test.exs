...
defmodule Ecto.Query.PlannerTest do
  ...
  test "plan: generates a cache key" do
    ...
    assert key == [:all, {"posts", Post, 116_318_561, "my_prefix"}]
    ...
    test "on all" do
      ...
      assert [
               ...
               {:non_recursive_cte, "cte",
                [{"comments", Comment, _, nil}, {:select, {:&, _, [0]}}]}
             ] = cache
  ...
  test "normalize: many_to_many assoc join with schema and wheres" do
    ...
    assert inspect(query) =~
             "join: c2 in Ecto.Query.PlannerTest.CommentPost, on: c2.post_id == p1.id and c2.deleted == ^..."
  ...
  test "normalize: many_to_many assoc join without schema and wheres" do
    ...
    assert inspect(query) =~
             "join: c2 in \"comment_posts\", on: c2.post_id == p1.id and c2.deleted == ^..."
  ...
  test "normalize: select" do
    ...
    assert query.select.fields ==
             select_fields(
               [:id, :post_title, :text, :code, :posted, :visits, :links, :prefs, :meta, :metas],
               0
             )
