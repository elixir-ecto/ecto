...
defmodule Ecto.Query.PlannerTest do
  ...
  test "plan: nested joins associations" do
    ...
    assert {{"comments", _, _}, {"posts", _, _}, {"comments", _, _}, {"posts", _, _}} =
             query.sources
    ...
    assert {{"comments", _, _}, {"comments", _, _}, {"posts", _, _}, {"posts", _, _}} =
             query.sources
  ...
  test "plan: generates a cache key" do
    ...
    assert key == [:all, {"posts", Post, 36_606_244, "my_prefix"}]
    ...
    assert key == [
             ...
             {:join, [{:inner, {"comments", Comment, 38_292_156, "world"}, true}]},
             {"posts", Post, 36_606_244, "hello"},
             {:select, 1}
           ]
  end
  ...
  test "plan: normalizes prefixes" do
    ...
    assert query.sources ==
             {{"comments", Comment, "global"}, {"comments", Comment, "global"},
              {"posts", Ecto.Query.PlannerTest.Post, "my_prefix"}}
    ...
    assert query.sources ==
             {{"comments", Comment, "global"}, {"comments", Comment, "local"},
              {"posts", Ecto.Query.PlannerTest.Post, "local"}}
  end
  ...
  test "plan: CTE prefixes" do
    ...
    {%{with_ctes: with_expr} = query, _, _} =
      Comment |> with_cte("cte", as: ^from(c in Comment)) |> Map.put(:prefix, "global") |> plan()
  ...
  test "normalize: assoc join with wheres that have in filters" do
    {_query, params, _select} =
      from(post in Post,
        ...
        join: post in assoc(comment, :crazy_post_with_list)
      )
      |> normalize_with_params()
    ...
    assert Macro.to_string(hd(query.wheres).expr) ==
             "&0.id() == ^0 and &0.post_title() in ^(1, 2)"
  ...
  test "normalize: all does not allow bindings in order bys when having combinations" do
    assert_raise Ecto.QueryError,
                 ~r"cannot use bindings in `order_by` when using `union_all`",
                 fn ->
