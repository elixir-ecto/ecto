defmodule Ecto.MigrationTest do
  ...
  setup meta do
    ...
    {:ok, runner} =
      Runner.start_link(self(), TestRepo, direction, :up, %{level: false, sql: false})
  ...
  test "creates a reference" do
    assert references(:posts) == %Reference{table: "posts", column: :id, type: :bigserial}
    assert references("posts") == %Reference{table: "posts", column: :id, type: :bigserial}
  ...
  test "forward: creates a table" do
    ...
    assert last_command() == {:create, table, [{:add, :title, :string, []}]}
  end
  ...
  test "forward: create a table with custom primary key" do
    ...
    assert last_command() == {:create, table, [{:add, :uuid, :uuid, [primary_key: true]}]}
  end
  ...
  test "forward: creates an empty table" do
    ...
    assert last_command() == {:create, table, [{:add, :id, :bigserial, [primary_key: true]}]}
  end
  ...
  test "forward: alters a table" do
    ...
    assert last_command() ==
             {:alter, %Table{name: "posts"},
              [{:add, :summary, :text, []}, {:modify, :title, :text, []}, {:remove, :views}]}
  end
  ...
  test "forward: raise error when prefixes don't match" do
    assert_raise Ecto.MigrationError,
                 "the :prefix option `foo` does match the migrator prefix `bar`",
                 fn ->
                   create(table(:posts, prefix: "foo"))
                   flush()
                 end
  ...
  test "backward: alters a table" do
    ...
    assert last_command() ==
             {:alter, %Table{name: "posts"}, [{:remove, :extension}, {:remove, :summary}]}
  ...
  test "references foreign keys types must be the same as primary defaults" do
    ...
    assert references(:posts) == %Reference{table: "posts", column: :id, type: :binary_id}
  end
