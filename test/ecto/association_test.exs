defmodule Ecto.AssociationTest do
  ...
  defmodule Post do
    ...
    def awesome() do
      from row in __MODULE__, where: row.upvotes >= 1000
    end
  end
  ...
  defmodule Comment do
    ...
    schema "comments" do
      ...
      has_one :permalink, Permalink
      # belongs -> belongs
      has_one :post_author, through: [:post, :author]
      # belongs -> one
  ...
  defmodule Permalink do
    ...
    def special() do
      from row in __MODULE__, where: row.special
    end
  end
  ...
  defmodule AuthorPermalink do
    ...
    def active() do
      from row in __MODULE__, where: not row.deleted
    end
  end
  ...
  defmodule Author do
    ...
    schema "authors" do
      ...
      # many -> one
      has_many :posts_permalinks, through: [:posts, :permalink]
      ...
      many_to_many :special_permalinks, Permalink.special(), join_through: AuthorPermalink
      many_to_many :active_permalinks, Permalink, join_through: AuthorPermalink.active()
      has_many :posts_with_prefix, PostWithPrefix
      has_many :comments_with_prefix, through: [:posts_with_prefix, :comments_with_prefix]
    end

    def super_users() do
      from row in __MODULE__, where: row.super_user
    end
  end
  ...
  test "has many with specified source" do
    ...
    assert inspect(Ecto.Association.Has.joins_query(assoc)) ==
             inspect(
               from a in Author, join: e in {"users_emails", Email}, on: e.author_id == a.id
             )
  ...
  test "has many query-based assoc query" do
    ...
    assert inspect(Ecto.Association.Has.joins_query(assoc)) ==
             inspect(
               from a in Author,
                 join: p in ^from(p in Post, where: p.upvotes >= 1000),
                 on: p.author_id == a.id
             )
  ...
  test "has many query-based assoc with custom query" do
    ...
    assert inspect(Ecto.Association.Has.assoc_query(assoc, query, [1, 2, 3])) ==
             inspect(
               from p in Post,
                 where: p.upvotes >= 1000,
                 where: p.author_id in ^[1, 2, 3],
                 limit: 5
             )
  end
  ...
  test "has one with specified source" do
    ...
    assert inspect(Ecto.Association.Has.joins_query(assoc)) ==
             inspect(
               from a in Author, join: p in {"users_profiles", Profile}, on: p.author_id == a.id
             )
  ...
  test "belongs to with specified source" do
    ...
    assert inspect(Ecto.Association.BelongsTo.joins_query(assoc)) ==
             inspect(
               from e in Email, join: a in {"post_authors", Author}, on: a.id == e.author_id
             )
  ...
  test "belongs to query-based assoc query" do
    ...
    assert inspect(Ecto.Association.BelongsTo.joins_query(assoc)) ==
             inspect(
               from e in Email,
                 join: a in ^from(a in Author, where: a.super_user),
                 on: a.id == e.author_id
             )
  ...
  test "many to many with specified source" do
    ...
    assert inspect(Ecto.Association.ManyToMany.joins_query(assoc)) ==
             inspect(
               from a in Author,
                 join: m in "authors_permalinks",
                 on: m.author_id == a.id,
                 join: p in {"custom_permalinks", Permalink},
                 on: m.permalink_id == p.id
             )

    assert inspect(Ecto.Association.ManyToMany.assoc_query(assoc, nil, [])) ==
             inspect(
               from p in {"custom_permalinks", Permalink},
                 join: a in Author,
                 on: a.id in ^[],
                 join: m in "authors_permalinks",
                 on: m.author_id == a.id,
  ...
  test "has many through many to many" do
    ...
    assert inspect(Ecto.Association.HasThrough.joins_query(assoc)) ==
             inspect(
               from a in Author, join: p in assoc(a, :posts), join: c in assoc(p, :comments)
             )
  ...
  test "has many through many to one" do
    ...
    assert inspect(Ecto.Association.HasThrough.joins_query(assoc)) ==
             inspect(
               from a in Author, join: p in assoc(a, :posts), join: c in assoc(p, :permalink)
             )
  ...
  test "has one through belongs to one" do
    ...
    assert inspect(Ecto.Association.HasThrough.joins_query(assoc)) ==
             inspect(
               from c in Comment, join: p in assoc(c, :post), join: l in assoc(p, :permalink)
             )
  ...
  test "has many through one to many" do
    ...
    assert inspect(Ecto.Association.HasThrough.joins_query(assoc)) ==
             inspect(
               from s in Summary, join: p in assoc(s, :post), join: c in assoc(p, :comments)
             )
  ...
  test "has many through custom assoc many to many query" do
    ...
    assert inspect(Ecto.Association.HasThrough.assoc_query(assoc, query, [1, 2, 3])) ==
             inspect(
               from c in Comment,
                 join: p0 in Permalink,
                 on: p0.comment_id == c.id,
                 join: p1 in Post,
                 on: p1.author_id in ^[1, 2, 3],
  ...
  test "has many through many to many and has many" do
    ...
    assert inspect(Ecto.Association.HasThrough.assoc_query(assoc, nil, [1, 2, 3])) ==
             inspect(
               from e in {"users_emails", Email},
                 join: p in Permalink,
                 on: p.id in ^[1, 2, 3],
                 join: ap in AuthorPermalink,
                 on: ap.permalink_id == p.id,
                 join: a in Author,
  ...
  test "build/2" do
    # has many
    assert build_assoc(%Post{id: 1}, :comments) == %Comment{post_id: 1}

    # has one
    assert build_assoc(%Summary{id: 1}, :post) == %Post{summary_id: 1, title: "default"}

    # belongs to
    assert build_assoc(%Post{id: 1}, :author) == %Author{title: "World!"}

    # many to many
    assert build_assoc(%Permalink{id: 1}, :authors) == %Author{title: "m2m!"}
  ...
  test "build/3 with custom attributes" do
    ...
    # has one
    assert build_assoc(%Post{id: 1}, :comments, post_id: 2) == %Comment{post_id: 1}

    # belongs to
    assert build_assoc(%Post{id: 1}, :author, title: "Hello!") == %Author{title: "Hello!"}
    ...
    # many to many
    assert build_assoc(%Permalink{id: 1}, :authors, title: "Hello!") == %Author{title: "Hello!"}
  ...
  test "preload: normalizer" do
    assert Preloader.normalize(:foo, nil, []) == [foo: {nil, nil, []}]
    assert Preloader.normalize([foo: :bar], nil, []) == [foo: {nil, nil, [bar: {nil, nil, []}]}]

    assert Preloader.normalize([foo: [:bar, baz: :bat], this: :that], nil, []) ==
             [
               this: {nil, nil, [that: {nil, nil, []}]},
               foo: {nil, nil, [baz: {nil, nil, [bat: {nil, nil, []}]}, bar: {nil, nil, []}]}
             ]
  end

  test "preload: normalize with query" do
    query = from(p in Post, limit: 1)
    assert Preloader.normalize([foo: query], nil, []) == [foo: {nil, query, []}]
  ...
  test "preload: normalize with take" do
    assert Preloader.normalize([:foo], [foo: :id], []) == [foo: {[:id], nil, []}]
  ...
  test "preload: expand" do
    ...
    assert {%{
              post:
                {{:assoc, %Ecto.Association.BelongsTo{}, {0, :id}}, nil, nil,
                 [author: {nil, nil, []}]}
    ...
    assert {%{
              posts:
                {{:assoc, %Ecto.Association.Has{}, {0, :author_id}}, nil, nil,
  ...
  test "preload: expand with take" do
    assert {%{
              permalink: {{:assoc, %Ecto.Association.Has{}, {0, :post_id}}, [:id], nil, []},
              comments: {{:assoc, %Ecto.Association.Has{}, {0, :post_id}}, nil, nil, []}
  ...
  test "preload: expand raises on duplicated entries" do
    message =
      ~r"cannot preload `comments` as it has been supplied more than once with different queries"

    assert_raise ArgumentError, message, fn ->
      expand(
        Post,
        comments: from(c in Comment, limit: 2),
  ...
  describe "after_compile_validation/2" do
    defp after_compile_validation(assoc, name, opts) do
      defmodule Sample do
        ...
        schema "sample" do
          ...
          throw(
            assoc.after_compile_validation(assoc.struct(__MODULE__, name, opts), %{
              __ENV__
              | context_modules: [Ecto.AssociationTest]
            })
          )
        end
      end
    ...
    test "for has" do
      assert after_compile_validation(
      ...
      assert after_compile_validation(
               ...
               {:error,
                "associated schema Ecto.AssociationTest.Post does not have field `sample_id`"}
    ...
    test "for belongs_to" do
      ...
      assert after_compile_validation(
               ...
               {:error,
                "associated schema Ecto.AssociationTest.Post does not have field `non_id`"}
