...
defmodule Ecto.SchemaTest do
  ...
  defmodule CustomSchema do
    ...
    @field_source_mapper fn field ->
      field |> Atom.to_string() |> String.upcase() |> String.to_atom()
    end
  ...
  test "timestamps autogenerate metadata (private)" do
    ...
    assert Timestamps.__schema__(:autoupdate) == [updated_at: {:m, :f, [:a]}]
  end
  ...
  test "has_* expects a queryable" do
    message =
      ~r"association :posts queryable must be a schema, a {source, schema}, or a query. got: 123"
