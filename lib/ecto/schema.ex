defmodule Ecto.Schema do
  ...
  **Important!** Please use this feature only when strictly necessary,
      ...
      case gen = opts[:autogenerate] do
        ...
        autogenerate_opts when is_list(autogenerate_opts) ->
          if autogenerate_id?(type) do
            store_autogenerate_id!(mod, name, source || name, type, pk?, autogenerate_opts)
          else
  ...
  defp store_autogenerate_id!(mod, name, source, type, pk?, opts \\ []) do
    cond do
      ...
      true ->
        autogen_tuple = if opts == [], do: {name, source, type}, else: {name, source, type, opts}
        Module.put_attribute(mod, :ecto_autogenerate_id, autogen_tuple)
        true
    end
  end
