defmodule Ecto.Migration do
  ...
  @doc """
  Returns a table struct that can be given to create, alter, etc.
  ...
  ## Options
    ...
    * `:concurrently` - indicates whether the index should be created/dropped
      concurrently.
    * `:using` - configures the index type (for PostgreSQL).
  ...
  **Note**: in order for the `:concurrently` option to work, the migration must
  ...
  PostgreSQL supports several index types like B-tree, Hash, and GiST. When
  ...
  value is passed to the generated `USING` clause as is.
  ...
  ## Options
    ...
    * `:default` - the column's default value. Can be a string, number, empty
