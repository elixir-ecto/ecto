defmodule Ecto.Migration do
  ...
  **Note**: in order for the `:concurrently` option to work, the migration must
  ...
  ## Options
    ...
    * `:size` - the size of the type, see below for default values
    * `:precision` - the precision for a numberic type. Default is no precision
    * `:scale` - the scale of a numberic type. Default is 0 scale

  ## Size

  By default, most columns have no size, with the exception of:

    * `:string` - by default have a size of 255 chars
    * `:binary` - by default have a size of 1 megabyte (i.e. 1_048_576 bytes)

  """
  def add(column, type \\ :string, opts \\ []) when is_atom(column) do
    if size = size_for_type(type) do
      opts = Keyword.put_new(opts, :size, size)
    end
    Runner.subcommand {:add, column, type, opts}
  end

  defp size_for_type(:string), do: 255
  defp size_for_type(:binary), do: 1_048_576
  defp size_for_type(_),       do: nil
