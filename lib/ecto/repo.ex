defmodule Ecto.Repo do
  ...
  the database:
      ...
      # a column on conflicts. In case of success, the operation returns
      # :ok and the existing ID in the database will be returned as long
      # as the ID is autogenerated by the database.
      #
      # Keep in mind other columns may not necessarily reflect the data
      # in the database. In fact, any operation done on `:on_conflict`
      ...
      # In case you want to update the struct with certain columns coming
      # from the database, you can mark those columns with :read_after_writes
      # in your schema definition or pass a list of columns to :returning.
      # This option is not supported by MySQL, unfortunately.
      {:ok, updated} = MyRepo.insert(%Post{title: "this is unique"}, returning: [:body],
                                     on_conflict: on_conflict, conflict_target: :title)

      # Alternatively you can use `returning: true` to get all DB columns back.
      {:ok, updated} = MyRepo.insert(%Post{title: "this is unique"}, returning: true,
                                     on_conflict: on_conflict, conflict_target: :title)
