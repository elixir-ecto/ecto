...
defmodule Ecto.Adapters.Postgres do
  ...
  def ddl_exists?(repo, object, opts) do
    {[{count}], _} = query(repo, SQL.ddl_exists_query(object), [], opts)
    count > 0
  end

  # TODO: If this module is not loaded, raise with a meaningful error
  if Code.ensure_loaded?(Postgrex.Connection) do

    # Implementation for Ecto.Adapter.SQL.Connection
    defmodule Connection do
      @moduledoc false

      @behaviour Ecto.Adapter.SQL.Connection
      @default_port 5432
