defmodule Ecto.Repo.Schema do
  ...
  defp do_insert(repo, adapter, %Changeset{valid?: true} = changeset, opts) do
    ...
    wrap_in_transaction(data, adapter, opts, assocs, prepare, fn ->
      ...
      if changeset.valid? do
        ...
        args = [data, metadata, changes, on_conflict, return, opts]
    ...
    if changeset.changes != %{} or force? do
      ...
      wrap_in_transaction(data, adapter, opts, assocs, prepare, fn ->
        ...
        if changeset.valid? do
          ...
          args    = [data, metadata(struct, opts), changes, filters, return, opts]
  ...
  defp do_delete(repo, adapter, %Changeset{valid?: true} = changeset, opts) do
    ...
    wrap_in_transaction(data, adapter, opts, assocs, prepare, fn ->
      ...
      args = [data, metadata(struct, opts), filters, opts]
