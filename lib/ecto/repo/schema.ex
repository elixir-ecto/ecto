    ...
    if changeset.changes != %{} or force? do
      wrap_in_transaction(adapter, adapter_meta, opts, changeset, assocs, embeds, prepare, fn ->
        ...
        if changeset.valid? do
          ...
          types =
            for {name, _} <- (Enum.to_list(changes) ++ Enum.to_list(filters)) do
              case Map.fetch!(changeset.types, name) do
                # translate Ecto :id type to the underlying tds type, integer.
                :id -> :integer
                other -> other
              end
            end
