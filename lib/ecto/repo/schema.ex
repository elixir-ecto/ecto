    ...
    if changeset.changes != %{} or force? do
      wrap_in_transaction(adapter, adapter_meta, opts, assocs, prepare, fn ->
        ...
        if changeset.valid? do
          ...
          {action, query, autogen} =
            if original != %{} or (autogen != [] and force?) do
              changes = autogen ++ Keyword.new(original)
              ...
              if return_sources != [] do
                select(query, ^return_sources)
              else
                query
              end

              {query, _params} = Ecto.Adapter.plan_query(:update_all, adapter, query)

              {:update, query, autogen}
            else
              {:noop, nil, []}
            end

          args = [adapter_meta, query, opts]
