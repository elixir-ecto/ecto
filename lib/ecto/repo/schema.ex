defmodule Ecto.Repo.Schema do
          ...
          {action, autogen} =
            if original != %{} ||
                 forced_update?(
                   ...
                   children,
                   parents,
                   opts[:force_update_on_relation_change]
                 ),
               do: {:update, autogen},
               else: {:noop, []}
  ...
  defp forced_update?(autogen, force?, children, parents, forced_relations) do
    cond do
      autogen == [] -> false
      force? -> true
      is_nil(forced_relations) -> false
      true ->
        Enum.any?(children, fn {assoc, _} -> assoc.field in forced_relations end) ||
          Enum.any?(parents, fn {assoc, _} -> assoc.field in forced_relations end)
    end
  end
