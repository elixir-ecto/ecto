defmodule Ecto.Repo.Schema do
  ...
  defp autogenerate_id({key, source, type}, fields, header, adapter) do
    autogenerate_id({key, source, type, []}, fields, header, adapter)
  end

  defp autogenerate_id({key, source, type, opts}, fields, header, adapter) do
    case :lists.keyfind(key, 1, fields) do
      {^key, _, _} ->
        {fields, header}

      false ->
        if dump_value = Ecto.Type.adapter_autogenerate(adapter, type, opts) do
  ...
  defp autogenerate_id({key, source, type}, changes, return_types, return_sources, adapter) do
    autogenerate_id({key, source, type, []}, changes, return_types, return_sources, adapter)
  end

  defp autogenerate_id({key, source, type, opts}, changes, return_types, return_sources, adapter) do
    cond do
      # Set by user
      ...
      # Autogenerated now
      dump_value = Ecto.Type.adapter_autogenerate(adapter, type, opts) ->
        {:ok, cast_value} = Ecto.Type.adapter_load(adapter, type, dump_value)
