defmodule Ecto.Repo.Schema do
  ...
  defp do_insert(repo, _name, %Changeset{valid?: false} = changeset, _opts) do
    ...
    if changeset.changes != %{} or force? do
      wrap_in_transaction(adapter, adapter_meta, opts, changeset, assocs, embeds, prepare, fn ->
        ...
        if changeset.valid? do
          ...
          filters = add_pk_type(filters, schema)
  ...
  defp do_update(repo, _name, %Changeset{valid?: false} = changeset, _opts) do
  ...
  defp do_delete(repo, _name, %Changeset{valid?: false} = changeset, _opts) do
  ...
  defp add_pk_type(filters, schema) do
    pk_types =
      for pk <- schema.__schema__(:primary_key),
          type = Ecto.Type.type(schema.__schema__(:type, pk)),
          ! Ecto.Type.base?(type) do
        {pk, type}
      end

    Enum.reduce(pk_types, filters, fn
      {pk, type}, filters -> Enum.map(filters, fn
        {^pk, val} -> {pk, val, type}
        any -> any
      end)
    end)
