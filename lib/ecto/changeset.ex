defmodule Ecto.Changeset do
  ...
  ## Options
    ...
    * `:messages` - a keyword list of error messages to use when a parameter cannot be cast to
      the given type. Keys must correspond to fields being passed into this function. This
      will override the default message generated by Ecto as well as error messages defined
      by custom types
  ...
  You can define custom error messages on a per-field basis.

      # Using default error message
      iex> params = %{title: 1, body: ""}
      iex> changeset = cast(post, params, [:title, :body])
      iex> changeset.errors
      changeset.errors == [title: {"is invalid", [type: :string, validation: :cast]}]

      # Providing a custom error message
      iex> params = %{title: 1, body: ""}
      iex> changeset = cast(post, params, [:title, :body], messages: [title: "must be a string"])
      iex> changeset.errors
      changeset.errors == [title: {"must be a string", [type: :string, validation: :cast]}]
  ...
  defp cast(%{} = data, %{} = types, %{} = changes, %{} = params, permitted, opts) when is_list(permitted) do
    ...
    {changes, errors, valid?} =
      Enum.reduce(permitted, {changes, [], true},
                  &process_param(&1, params, types, data, empty_values, defaults, force?, messages, &2))
  ...
  defp process_param(key, params, types, data, empty_values, defaults, force?, messages, {changes, errors, valid?}) do
