...
defimpl Inspect, for: Ecto.Query.DynamicExpr do
  def inspect(%DynamicExpr{binding: binding} = dynamic, opts) do
    ...
    surround_many("dynamic(", [Macro.to_string(binding), inspected], ")", opts, fn str, _ ->
      str
    end)
  end
end
...
defimpl Inspect, for: Ecto.Query do
  ...
  defp join(
         %JoinExpr{qual: qual, source: {:fragment, _, _} = source, on: on, as: as} = part,
         name,
         names
       ) do
  ...
  defp normalize_source("Elixir." <> _ = source), do: source |> Module.split() |> List.last()
