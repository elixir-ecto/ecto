...
defmodule Ecto.Query.Builder.OrderBy do
  ...
  defp do_escape({dir, {:^, _, [expr]}}, params, kind, _vars, _env) do
    {{quoted_dir!(kind, dir),
      quote(do: Ecto.Query.Builder.OrderBy.field!(unquote(kind), unquote(expr)))}, params}
  end
  ...
  def quoted_dir!(_kind, dir) when dir in [:asc, :desc], do: dir

  def quoted_dir!(kind, other),
    do:
      Builder.error!(
        "expected :asc, :desc or interpolated value in `#{kind}`, got: `#{inspect(other)}`"
      )

  @doc """
  Called by at runtime to verify the direction.
  """
  def dir!(_kind, dir) when dir in [:asc, :desc], do: dir
  ...
  def field!(kind, other) do
    raise ArgumentError,
          "expected a field as an atom, a list or keyword list in `#{kind}`, got: `#{
            inspect(other)
          }`"
  end
