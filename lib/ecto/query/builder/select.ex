...
defmodule Ecto.Query.Builder.Select do
  ...
  defp merge(query, select, old_expr, old_params, old_take, new_select) do
    ...
    expr =
      case {old_expr, new_expr} do
        ...
        {{:%{}, meta, old_fields}, {:%{}, _, new_fields}}
        when (old_fields == [] or tuple_size(hd(old_fields)) == 2) and
               (new_fields == [] or tuple_size(hd(new_fields)) == 2) and old_params == [] ->
          {:%{}, meta, Keyword.merge(old_fields, new_fields)}

        {{:%, meta, [name, {:%{}, meta, old_fields}]}, {:%{}, _, new_fields}}
        when (old_fields == [] or tuple_size(hd(old_fields)) == 2) and
               (new_fields == [] or tuple_size(hd(new_fields)) == 2) and old_params == [] ->
        ...
        {_, _} ->
          raise Ecto.QueryError,
            query: query,
            message:
              "cannot select_merge #{merge_argument_to_error(new_expr)} into " <>
  ...
  defp merge_take_kind(binding, old, new) do
    Builder.error!(
      "cannot apply select_merge because the binding at position #{binding} " <>
