defmodule Ecto.Query.Planner do
  ...
  defp prepare_source(_query, {source, schema}, _adapter)
       when is_binary(source) and is_atom(schema),
       do: {source, schema}

  defp prepare_source(_query, {:fragment, _, _} = source, _adapter), do: source
  ...
  defp subquery_select(expr, _take, query) do
    error!(
      query,
      "subquery must select a source (t), a field (t.field) or a map, got: `#{
  ...
  defp assert_subquery_fields!(query, expr, pairs) do
    Enum.each(pairs, fn
      {key, _} when not is_atom(key) ->
        error!(
          query,
          "only atom keys are allowed when selecting a map in subquery, got: `#{
            Macro.to_string(expr)
          }`"
        )
      ...
      {key, value} ->
        if valid_subquery_value?(value) do
          ...
          error!(
            query,
            "maps, lists, tuples and sources are not allowed as map values in subquery, got: `#{
              Macro.to_string(expr)
            }`"
          )
        end
  ...
  defp prepare_joins(
         [%JoinExpr{assoc: {ix, assoc}, qual: qual, on: on} = join | t],
         query,
         joins,
         sources,
         tail_sources,
  ...
  defp prepare_joins(
         [%JoinExpr{source: %Ecto.Query{} = join_query, qual: qual, on: on} = join | t],
         query,
         joins,
         sources,
         tail_sources,
  ...
  defp rewrite_join(%{on: on, ix: join_ix} = join, qual, ix, last_ix, source_ix, inc_ix) do
    ...
    %{join | on: on, qual: qual, ix: rewrite_ix(join_ix, ix, last_ix, source_ix, inc_ix)}
  end
  ...
  defp merge_cache(:join, query, exprs, {cache, params}, adapter) do
    {expr_cache, {params, cacheable?}} =
      Enum.map_reduce(exprs, {params, true}, fn %JoinExpr{on: on, qual: qual, source: source} =
                                                  join,
                                                {params, cacheable?} ->
        {key, params} = source_cache(source, params)
  ...
  defp finalize_cache(
         %{assocs: assocs, prefix: prefix, lock: lock, select: select},
         operation,
         cache,
         counter
       ) do
  ...
  defp source_cache({_, nil} = source, params), do: {source, params}
  defp source_cache({bin, schema}, params), do: {{bin, schema, schema.__schema__(:hash)}, params}
  defp source_cache({:fragment, _, _} = source, params), do: {source, params}
  ...
  defp cast_param(_kind, query, expr, %DynamicExpr{}, _type, _value) do
    error!(
      query,
      expr,
      "dynamic expressions can only be interpolated inside other " <>
  ...
  defp cast_param(_kind, query, expr, [{_, _} | _], _type, _value) do
    error!(
      query,
      expr,
      "keyword lists can only be interpolated at the top level of " <>
  ...
  defp prepare_assocs(query, ix, assocs) do
    ...
    Enum.each(assocs, fn {assoc, {child_ix, child_assocs}} ->
      ...
      unless refl do
        error!(
          query,
          "field `#{inspect(parent_schema)}.#{assoc}` " <> "in preload is not an association"
        )
      end
  ...
  defp prewalk(
         {{:., dot_meta, [{:&, amp_meta, [ix]}, field]}, meta, []},
         kind,
         query,
         _expr,
         acc,
  ...
  defp dump_param(kind, type, v, adapter) do
    with {:ok, type} <- normalize_param(kind, type, v), do: dump_param(adapter, type, v)
  end
  ...
  defp normalize_select(query) do
    ...
    {postprocess, fields, from} = collect_fields(expr, [], :error, query, take)
  ...
  defp take!(source, query, fetched, field, ix) do
    case {fetched, source} do
      ...
      {{:ok, {_, _}}, {:fragment, _, _}} ->
        error!(
          query,
          "it is not possible to return a map/struct subset of a fragment, " <>
      ...
      {{:ok, {_, _}}, %Ecto.SubQuery{}} ->
        error!(
          query,
          "it is not possible to return a map/struct subset of a subquery, " <>
  ...
  defp assert_only_filter_expressions!(query, operation) do
    case query do
      ...
      _ ->
        error!(
          query,
          "`#{operation}` allows only `where` and `join` expressions. " <>
