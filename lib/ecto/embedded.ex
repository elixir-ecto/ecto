defmodule Ecto.Embedded do
  ...
  @doc """
  Helper for loading an embed.

  This function may raise when there are errors when
  loading nested embeds.
  """
  def load(embed, value, adapter, fun)

  def load(_, nil, _adapter, _fun) do
    {:ok, nil}
  end

  def load(%{cardinality: :one, embed: model, field: field},
           value, adapter, fun) when is_map(value) do
    load(field, model, value, adapter, fun)
  end

  def load(%{cardinality: :many, container: :array, embed: model, field: field},
           value, adapter, fun) when is_list(value) do
    Enum.map(value, &load(field, model, &1, adapter, fun))
  end

  def load(_embed, _value, _adapter, _fun) do
    :error
  end

  defp load(_field, model, value, adapter, loader) when is_map(value) do
    {:ok,
      Ecto.Schema.Serializer.load!(model, nil, nil, value, fn
        # Go recursive through the adapter
        {:embed, _} = type, value -> adapter.load(type, value)

        # Using the custom loader
        type, value -> loader.(type, value)
      end)}
  end

  defp load(field, _model, value, _adapter, _fun) do
    raise ArgumentError, "cannot load embed `#{field}`, invalid value: #{inspect value}"
  end

  @doc """
  Helper for dumping an embed.

  This function may raise when there are errors when
  dumping nested embeds.
  """
  def dump(embed, value, adapter, fun)

  def dump(_, nil, _adapter, _fun) do
    {:ok, nil}
  end

  def dump(%{cardinality: :one, embed: model, field: field},
           value, adapter, fun) when is_map(value) do
    dump(field, model, value, adapter, fun)
  end

  def dump(%{cardinality: :many, container: :array, embed: model, field: field},
           value, adapter, fun) when is_list(value) do
    Enum.map(value, &dump(field, model, &1, adapter, fun))
  end

  def dump(_embed, _value, _adapter, _fun) do
    :error
  end

  defp dump(field, model, %Ecto.Changeset{} = changeset, adapter, fun) do
    dump(field, model, Ecto.Changeset.apply_changes(changeset), adapter, fun)
  end

  defp dump(_field, model, %{__struct__: model} = value, adapter, dumper) when is_map(value) do
    {:ok,
      Ecto.Schema.Serializer.dump!(model, value, fn
        # Go recursive through the adapter
        {:embed, _} = type, value -> adapter.dump(type, value)

        # Using the custom loader
        type, value -> dumper.(type, value)
      end)}
  end

  defp dump(field, _model, value, _adapter, _fun) do
