defmodule Ecto.Integration.IntervalTest do
  ...
  test "date_add with hour" do
    ...
    assert [~D[2014-01-03]] =
             TestRepo.all(from p in Post, select: date_add(p.posted, ^48, "hour"))

    assert [~D[2014-01-03]] =
             TestRepo.all(from p in Post, select: date_add(p.posted, ^48.0, "hour"))

    assert [~D[2014-01-03]] =
             TestRepo.all(from p in Post, select: date_add(p.posted, ^dec, "hour"))
  end
  ...
  test "datetime_add with millisecond" do
    ...
    assert [~N[2014-01-01 02:00:01]] =
             TestRepo.all(
               from p in Post, select: datetime_add(p.inserted_at, ^dec, "millisecond")
             )
  end
  ...
  test "datetime_add with microsecond" do
    ...
    assert [~N[2014-01-01 02:00:00]] =
             TestRepo.all(
               from p in Post, select: datetime_add(p.inserted_at, 1500.0, "microsecond")
             )
    ...
    assert [~N[2014-01-01 02:00:00]] =
             TestRepo.all(
               from p in Post, select: datetime_add(p.inserted_at, ^dec, "microsecond")
             )
  end
  ...
  test "datetime_add with dynamic in filters" do
    ...
    assert [_] =
             TestRepo.all(
               from p in Post, where: p.inserted_at > datetime_add(^inserted_at, ^(-1), "year")
             )

    assert [_] =
             TestRepo.all(
               from p in Post, where: p.inserted_at > datetime_add(^inserted_at, -3, "month")
             )

    assert [_] =
             TestRepo.all(
               from p in Post,
                 where: field(p, ^field) > datetime_add(^inserted_at, ^(-3), ^"week")
             )

    assert [_] =
             TestRepo.all(
               from p in Post, where: field(p, ^field) > datetime_add(^inserted_at, -5, ^"day")
             )
  end
  ...
  test "datetime_add with naive_datetime_usec" do
    ...
    assert [^datetime] =
             TestRepo.all(
               from u in Usec,
                 select: datetime_add(type(^datetime, :naive_datetime_usec), 0, "microsecond")
             )

    assert [^datetime] =
             TestRepo.all(
               from u in Usec, select: datetime_add(u.naive_datetime_usec, 1500, "microsecond")
             )

    assert [^datetime] =
             TestRepo.all(
               from u in Usec, select: datetime_add(u.naive_datetime_usec, 1500.0, "microsecond")
             )
    ...
    assert [^datetime] =
             TestRepo.all(
               from u in Usec, select: datetime_add(u.naive_datetime_usec, ^dec, "microsecond")
             )
  end
  ...
  test "datetime_add with utc_datetime_usec" do
    ...
    assert [^datetime] =
             TestRepo.all(
               from u in Usec,
                 select: datetime_add(type(^datetime, :utc_datetime_usec), 0, "microsecond")
             )

    assert [^datetime] =
             TestRepo.all(
               from u in Usec, select: datetime_add(u.utc_datetime_usec, 1500, "microsecond")
             )

    assert [^datetime] =
             TestRepo.all(
               from u in Usec, select: datetime_add(u.utc_datetime_usec, 1500.0, "microsecond")
             )
    ...
    assert [^datetime] =
             TestRepo.all(
               from u in Usec, select: datetime_add(u.utc_datetime_usec, ^dec, "microsecond")
             )
  end
end
