...
defmodule Ecto.Integration.RepoTest do
 ...
 @tag :foreign_key_constraint
  ...
  @tag :upsert
  test "upsert update autoupdate columns but not autogenerate" do
    {:ok, inserted} = TestRepo.upsert(%Post{title: "original"})
    {:ok, updated} = TestRepo.upsert(%Post{id: inserted.id, title: "updated"})

    # The UUID and inserted_at fields are autogenerated when first
    # created. During the upsert, if the record already exists do not
    # update these columns.
    assert inserted.uuid == updated.uuid
    assert inserted.inserted_at == updated.inserted_at
    assert inserted.inserted_at != updated.updated_at
  end
end
