...
> NOTE: by using `Ecto.Model`, an `:id` field with type `:integer` is generated by default, which is the primary key of the Model. In most cases, the database adapter is expected to generate and set this primary key on the model. If you want to use a different type of primary key, or decide to set it yourself, implement your model as per the ["Using Models with Custom Primary Keys" wiki entry](https://github.com/elixir-lang/ecto/wiki/Using-Models-with-Custom-Primary-Keys)
