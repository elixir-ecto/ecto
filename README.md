...
A simple example can be found [in the Ecto git repo](https://github.com/elixir-lang/ecto/tree/master/examples/simple). You can read more about it in `Ecto.Repo` docs.
...
> NOTE: by using `Ecto.Model`, an `:id` field with type `:integer` is generated by default, which is the primary key of the Model. If you want to use a different type of primary key, you can declare custom `@schema_defaults` in a module, and use that in place of `Ecto.Model`. Consult the `Ecto.Model.Schema` documentation for usage instructions.
...
In order to generate a new migration you first need to define a `priv/0` function inside your repository pointing to a directory that will keep repo data. We recommend it to be placed inside the `priv` in your application directory:
...
defmodule Repo.CreatePosts do
  ...
  def up do
    ...
    execute "INSERT INTO migrations_test VALUES ('test')"
  end
...
You can also write migrations using `change/0` and Ecto will infer how to reverse your migration if needed.
